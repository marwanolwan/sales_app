...
require_permission('manage_promotions'); // استخدام نفس الصلاحية
...
// التحقق من وجود customer_id هو أمر أساسي
if (!$customer_id) {
    $_SESSION['error_message'] = "يجب تحديد عميل لعرض عقوده.";
    header("Location: index.php?page=promotions");
    exit();
}

// جلب بيانات العميل لعرض اسمه في العنوان
...
switch ($action) {
    ...
    case 'edit':
        ...
        if ($action == 'edit' && $contract_id) {
            ...
            if (!$contract_data) {
                $_SESSION['error_message'] = "العقد المطلوب للتعديل غير موجود.";
    ...
    case 'list':
    default:
        // جلب قائمة العقود الحالية لهذا العميل
...
// تحميل القالب الرئيسي الذي سيقوم بدوره بتضمين الواجهة المحددة
